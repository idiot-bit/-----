...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add your custom message logic here
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_callback))  # Catch-all callback
    ...
    # Manual uploads (private chat documents)
    app.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & filters.Document.ALL,
        handle_document
    ))

    # Method 2 Key Input — filtered to short key-like input
    app.add_handler(MessageHandler(
        filters.TEXT & filters.Regex(r"^[\w\-]{4,20}$") & filters.User(user_id=list(ALLOWED_USERS) + [OWNER_ID]),
        handle_key_input
    ))

    # General text fallback (for all users including allowed/owner)
    app.add_handler(MessageHandler(
