...
async def handle_backup_restore_from_document(doc, context, user_id):
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            zip_ref.extractall(".")  # This overwrites config/state

        # Step 4: Reload state and config
        load_state()
        with open("config.json") as f:
            new_config = json.load(f)

        # Optional: Reapply config updates if needed
        global OWNER_ID, ALLOWED_USERS, USER_DATA, AUTO_SETUP, BOT_ACTIVE, BOT_ADMIN_LINK
        OWNER_ID = new_config.get("owner_id")
        ALLOWED_USERS = set(new_config.get("allowed_users", []))
        USER_DATA = new_config.get("user_data", {})
        AUTO_SETUP = new_config.get("auto_setup", {})
        BOT_ADMIN_LINK = new_config.get("bot_admin_link", "")
        BOT_ACTIVE = new_config.get("bot_active", True)
        ...
        await context.bot.edit_message_text(
            ...
            text="✅ Backup restored successfully!\nPlease restart the bot manually on Railway or Termux to apply changes."
        )
