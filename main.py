    ...
    try:
        ...
        keyboard = [
            ...
            [
                ...
                InlineKeyboardButton("🔤 Add Code Style", callback_data="method2_mono")
            ],
            [
...
async def key_after_apks(user_id, context):
    try:
        ...
        for msg_id in session.get("post_message_ids", []):
            try:
                await context.bot.delete_message(chat_id=channel_id, message_id=msg_id)
            except:
                pass

        # Step 1: Send all APKs without caption
        media = [InputMediaDocument(media=fid) for fid in file_ids]
        new_posts = await context.bot.send_media_group(chat_id=channel_id, media=media)

        # Step 2: Format and send key as separate message
        if key_mode == "quote":
        ...
        # Final list of post message IDs
        post_ids = [m.message_id for m in new_posts] + [key_msg.message_id]
        last_msg = key_msg
        ...
        if preview_message_id:
            try:
                await context.bot.edit_message_text(
                    chat_id=user_id,
                    message_id=preview_message_id,
                    text="<b>Key sent separately after APKs ☑️</b>\n\nManage your post below:",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup(buttons)
                )
            except:
