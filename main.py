...
async def start_method2_countdown(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for idx, (name, fid) in enumerate(zip(filenames, file_ids), start=1):
            ...
            # Save size for preview
            if len(state["session_sizes"]) < idx:
                state["session_sizes"].append(size_str.replace(" MB", ""))
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if state.get("quote_applied"):
            await context.bot.send_message(
                chat_id=user_id,
                text="ℹ️ <b>Quote format is already applied!</b>",
                parse_mode="HTML"
            )
            return
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if state.get("mono_applied"):
            await context.bot.send_message(
                chat_id=user_id,
                text="ℹ️ <b>Mono format is already applied!</b>",
                parse_mode="HTML"
            )
            return
