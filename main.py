...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if state.get("current_method") == "method2":
            ...
            # BLOCK: Only accept key if waiting_key is explicitly True
            if not state.get("waiting_key", False):
                return
        
            # Step 1: Stop if key already saved
            ...
            # Step 8: Save key and reset flags
            state["saved_key"] = key
            state["waiting_key"] = False
            ...
            # Step 10: Save message ID for preview panel
            state["preview_message_id"] = sent.message_id
            return
