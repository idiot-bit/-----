...
# === Load config.json ===
def load_config():
    global OWNER_ID, ALLOWED_USERS, USER_DATA, BOT_ADMIN_LINK, BOT_ACTIVE, AUTO_SETUP
    if not os.path.exists("config.json"):
        return  # Fail-safe for first run
    ...
    AUTO_SETUP = config.get("auto_setup", AUTO_SETUP)
...
def load_state():
    ...
    if os.path.exists(STATE_FILE):
        with open(STATE_FILE, "r") as f:
            data = json.load(f)
            USER_STATE.update(data.get("user_state", {}))
            AUTO_SETUP.update(data.get("auto_setup", {}))
            USER_DATA.update(data.get("user_data", {}))

            saved_auto4 = data.get("auto4_state", {})
            AUTO4_STATE["pending_apks"] = saved_auto4.get("pending_apks", [])
            AUTO4_STATE["setup_mode"] = saved_auto4.get("setup_mode", 1)
            AUTO4_STATE["timer"] = None
            AUTO4_STATE["waiting_since"] = None
            AUTO4_STATE["countdown_msg_id"] = None

    # Ensure all user states have full keys
...
def reset_method2():
    AUTO4_STATE["pending_apks"] = []
    AUTO4_STATE["timer"] = None
    AUTO4_STATE["waiting_since"] = None
    AUTO4_STATE["countdown_msg_id"] = None
    AUTO4_STATE["setup_mode"] = 1
...
async def backup_config(context=None, query=None):
    ...
    with open("config.json", "w") as f:
        json.dump({
            ...
            "user_data": USER_DATA,
            "auto_setup": AUTO_SETUP,
            "bot_active": BOT_ACTIVE if isinstance(BOT_ACTIVE, bool) else True,
            "bot_admin_link": BOT_ADMIN_LINK
        }, f, indent=4)

    # Reset volatile fields before saving state.json
    AUTO4_STATE["timer"] = None
    AUTO4_STATE["waiting_since"] = None
    AUTO4_STATE["countdown_msg_id"] = None
    ...
    # Send ZIP to owner if context is provided
    if context:
        try:
            caption = (
    ...
    # Update message if triggered via inline button
    if query:
...
async def handle_key_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Allow early key input during countdown
    if state.get("countdown_task") and not state.get("waiting_key"):
        task = state["countdown_task"]
        if not task.done():
            task.cancel()

        # Remove countdown message if present
        msg_id = state.get("countdown_msg_id")
        if msg_id:
            try:
...
async def handle_backup_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.endswith(".zip"):
        return  # Ignore non-zip files silently
    ...
    try:
        ...
        # Step 1: Load state.json and update allowed_users
        try:
            with open("state.json", "r") as f:
                state_data = json.load(f)
                allowed_users.clear()
                allowed_users.update(state_data.get("allowed_users", {}))
        except Exception as e:
            await update.message.reply_text(f"Error loading state.json: {e}")
            return
    ...
    await update.message.reply_text(
        "✅ Backup restored successfully.\n\nPlease now *restart your Railway app* to apply all changes.",
        reply_markup=keyboard,
        parse_mode="Markdown"
    )
...
def main():
    ...
    # Fallback for other callback buttons
    app.add_handler(CallbackQueryHandler(handle_callback))

    # --- MESSAGE HANDLERS ---

    # 1. ZIP restore from owner only
    app.add_handler(MessageHandler(
        filters.Document.FileExtension("zip") & filters.User(user_id=OWNER_ID),
        handle_backup_restore
    ))

    # 2. Auto handler for APKs forwarded from a channel
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL,
        unified_auto_handler
    ))

    # 3. Manual APK upload in private chat
    app.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & filters.Document.ALL,
        handle_document
    ))

    # 4. Method 2 Key input — must come before general text!
    app.add_handler(MessageHandler(
        filters.TEXT & filters.User(ALLOWED_USERS),
        handle_key_input
    ))

    # 5. General caption setup / fallback text input
    app.add_handler(MessageHandler(
