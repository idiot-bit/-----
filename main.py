...
async def method2_send_to_channel(user_id, context):
    ...
    # Check if only 1 APK or multiple
    if len(posted_ids) == 1:
        # Only 1 APK uploaded - Session Ends
        ...
        # 2 or 3 APKs uploaded - Keep session alive for Auto-Recaption
        USER_STATE[user_id]["session_files"] = session_files
        USER_STATE[user_id]["waiting_key"] = False
        USER_STATE[user_id]["last_apk_time"] = None
        USER_STATE[user_id]["key_mode"] = key_mode
    ...
    if len(posted_ids) >= 2:
        # If 2-3 APKs posted ➔ Show Auto-Recaption Button
...
async def auto_recaption(user_id, context):
    ...
    # Step 2: Send new media group
    new_posts = await context.bot.send_media_group(
        chat_id=channel_id,
        media=media
    )

    # Step 3: Delete old posts
    ...
    # Step 4: Save new post data
    new_post_ids = [msg.message_id for msg in new_posts]
    USER_STATE[user_id]["apk_posts"] = new_post_ids
    ...
    # Step 5: Clear session after successful repost
    USER_STATE[user_id]["session_files"] = []
    USER_STATE[user_id]["saved_key"] = None
    USER_STATE[user_id]["waiting_key"] = False
    USER_STATE[user_id]["last_apk_time"] = None
    ...
    buttons = [
        ...
        [InlineKeyboardButton("📤 Send One APK", callback_data="send_apk_method1"),
         InlineKeyboardButton("📤 Send 2-3 APKs", callback_data="send_apk_method2")]
    ]
