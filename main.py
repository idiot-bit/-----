...
async def start_method2_countdown(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Build APK list with smart size format
        apk_lines = []
        for idx, (name, fid) in enumerate(zip(filenames, file_ids), start=1):
            ...
            # Store just the float MB value for later caption use
            if len(state["session_sizes"]) < idx:
                state["session_sizes"].append(file_size_mb if isinstance(file_size_mb, float) else "?")
