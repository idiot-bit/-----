...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method not in ["method1", "method2"]:
        await message.reply_text("❌ Invalid method selected. Please choose again.")
        return
    ...
    if data == "view_users":
        ...
        async def cancel_zip_restore():
            for elapsed in range(1, 21):
                ...
                # If user already uploaded the zip or cancelled, stop timer
                ...
                try:
                    ...
                    pass  # ignore message edit errors (e.g., message deleted manually)

            # Timeout reached — clear state and notify user
            state = USER_STATE.get(user_id, {})
            if state.get("awaiting_zip"):
                state.pop("awaiting_zip", None)
                state.pop("zip_timeout", None)
                state.pop("pending_restore_file", None)
                state.pop("zip_prompt_message_id", None)
        ...
        # Start timer task in the background
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message or not update.effective_user:
        return  # Skip if it's not a valid user message
    ...
    state = USER_STATE.get(user_id)
    if not state:
        await update.message.reply_text("⚠️ Session expired or not started. Please try again.")
        return
