...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Cancel old countdown if pending key
        ...
        session_sizes = state.setdefault("session_sizes", [])

        # Handle overflow (max 3 APKs)
        ...
        session_sizes.append(file_size_mb)
        ...
        # Prompt for key if 3 APKs collected
...
async def start_method2_countdown(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Initialize session_sizes if not already
        if "session_sizes" not in state:
            state["session_sizes"] = []

        session_sizes = state.setdefault("session_sizes", [])
        apk_lines = []

        for idx, (fid, name) in enumerate(zip(file_ids, filenames), start=1):
            try:
                if idx <= len(session_sizes) and isinstance(session_sizes[idx - 1], float):
                    size = session_sizes[idx - 1]
                else:
                    file_info = await context.bot.get_file(fid)
                    size = round(file_info.file_size / (1024 * 1024), 2)  # Size in MB
                    if len(session_sizes) < idx:
                        session_sizes.append(size)
                    else:
                        session_sizes[idx - 1] = size

                if size >= 1024:
                    size_str = f"{round(size / 1024, 2)} GB"
                else:
                    size_str = f"{size} MB"
            except:
                size_str = "—"
                if len(session_sizes) < idx:
                    session_sizes.append(0)
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not preview_message_id or not key or not session_files:
            await context.bot.send_message(
                chat_id=user_id,
                text="⚠️ <b>No active APK session found to convert!</b>",
                parse_mode="HTML"
            )
            return

        # Apply quote key style
        state["quote_applied"] = True
        state["key_mode"] = "quote"  # reflect style change
        ...
        for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
            if idx <= len(session_sizes) and isinstance(session_sizes[idx - 1], float):
                file_size = session_sizes[idx - 1]
            else:
                try:
                    file_info = await context.bot.get_file(file_id)
                    file_size = round(file_info.file_size / (1024 * 1024), 2)
                    session_sizes.append(file_size)
                except Exception as e:
                    print(f"Failed to fetch file size for preview: {e}")
                    file_size = "?"
                    session_sizes.append(file_size)
        ...
        try:
            ...
            print(f"[Quote Edit Error] User: {user_id} | {e}")
            await context.bot.send_message(
                chat_id=user_id,
                text="⚠️ <b>Failed to update preview message.</b>",
                parse_mode="HTML"
            )
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
            if idx <= len(session_sizes) and isinstance(session_sizes[idx - 1], float):
                ...
                try:
                    ...
                    print(f"Failed to fetch file size for mono preview: {e}")
                    file_size = "?"
                    session_sizes.append(file_size)
        ...
        try:
            ...
            print(f"[Mono Edit Error] User: {user_id} | {e}")
            await context.bot.send_message(
                chat_id=user_id,
                text="⚠️ <b>Failed to update preview message.</b>",
                parse_mode="HTML"
...
async def method2_show_preview(user_id, context):
    try:
        ...
        session_sizes = user_state.get("session_sizes", [])  # <-- Pre-collected from countdown
        ...
        for idx, (file_name, file_size) in enumerate(zip(session_filenames, session_sizes), start=1):
            preview_text += f"{idx}. {file_name} [{file_size} MB]\n"
