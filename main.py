...
async def method2_send_to_channel(user_id, context):
    ...
    if len(posted_ids) == 1:
        ...
        # After posting
        USER_STATE[user_id]["posted_files"] = session_files.copy()  # Save separately!
        USER_STATE[user_id]["session_files"] = []  # Clear session_files
...
async def auto_recaption(user_id, context):
    ...
    posted_files = state.get("posted_files", [])
    ...
    USER_STATE[user_id]["session_files"] = []  # Clear uploads memory
    USER_STATE[user_id]["apk_posts"] = [msg.message_id for msg in new_posts]  # Save new
    USER_STATE[user_id]["saved_key"] = None
    USER_STATE[user_id]["waiting_key"] = False
    USER_STATE[user_id]["last_apk_time"] = None
