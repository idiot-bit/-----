...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Method 2 Key Handler — FINAL VERSION (Fixed)
        if state.get("current_method") == "method2":
            # 1. Block future key input if key already saved and not actively editing or previewing
            if state.get("saved_key") and not state.get("waiting_key", False):
                return  # Block if already received key and not waiting
