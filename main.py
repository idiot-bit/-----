...
async def ask_to_share(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(
            ...
            text="<b>Ready to share?</b> 🤔\n<i>Do you want to post this to your channel?</i>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    ...
    try:
        ...
        if data == "share_yes":
            ...
            try:
                ...
                # Track stats
                for stat_key in ["hourly_apks", "daily_apks", "weekly_apks", "monthly_apks",
                                 "hourly_keys", "daily_keys", "weekly_keys", "monthly_keys"]:
                    USER_STATE[user_id][stat_key] = USER_STATE[user_id].get(stat_key, 0) + 1
        
                # Save last post id for future deletion
                USER_STATE[user_id]["last_post"] = {
                    "channel": pending["channel"],
                    "msg_id": result.message_id
                }
        
                # Show confirmation with [View Post] / [Delete]
                keyboard = [
                    [InlineKeyboardButton("🔗 View Post", url=f"https://t.me/c/{str(pending['channel'])[4:]}/{result.message_id}")],
                    [InlineKeyboardButton("🗑️ Delete Apk", callback_data="delete_last")]
                ]
                await query.edit_message_text(
                    "<b>✅ APK posted successfully!</b>",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
