...
async def backup_config(context=None, query=None):
    ...
    # Save current state (do not overwrite config.json)
    save_state()

    # Create ZIP without modifying original files
    try:
    ...
    # Notify in chat if from inline button
    if query:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Default reply to show it's working ---
    await update.message.reply_text(f"✅ Text received: <code>{message_text}</code>", parse_mode="HTML")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Anti-spam cooldown
    now = time.time()
    if not hasattr(context.application, "user_cooldowns"):
        context.application.user_cooldowns = {}
    if user_id in context.application.user_cooldowns and now - context.application.user_cooldowns[user_id] < 1:
        await query.answer("⌛ Wait a second...", show_alert=False)
        return
    context.application.user_cooldowns[user_id] = now

    await query.edit_message_text(f"🔘 You clicked: <code>{data}</code>", parse_mode="HTML")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_callback))  # fallback for all others

    # --- MESSAGE HANDLERS ---

    # 1. ZIP restore (only owner)
    app.add_handler(MessageHandler(
        filters.Document.FileExtension("zip") & filters.User(user_id=OWNER_ID),
        handle_backup_restore
    ))

    # 2. Forwarded APKs from channel
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL,
        unified_auto_handler
    ))

    # 3. Manual uploads in private chat
    app.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & filters.Document.ALL,
        handle_document
    ))

    # 4. Key input handler (Method 2) before general text
    app.add_handler(MessageHandler(
        filters.TEXT & filters.User(user_id=list(ALLOWED_USERS)),
        handle_key_input
    ))

    # 5. Fallback text input (only after other TEXT handlers)
    app.add_handler(MessageHandler(
