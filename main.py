...
async def backup_config(context=None, query=None):
    ...
    with zipfile.ZipFile(zip_filename, "w") as zipf:
        for filename in ["config.json", "main.py", "requirements.txt", "Procfile"]:
...
async def handle_backup_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.endswith(".zip"):
        return  # Ignore silently if not a ZIP

    state = USER_STATE.get(user_id, {})
    timeout = state.get("zip_timeout", 0)

    if not state.get("awaiting_zip") or time.time() > timeout:
        return  # Ignore if not awaiting or timeout expired
...
async def handle_backup_restore_from_document(doc, context, user_id):
    ...
    try:
        # Safety: backup current files
        if os.path.exists("config.json"):
            shutil.copy("config.json", "config_backup_before_restore.json")
        if os.path.exists("state.json"):
            shutil.copy("state.json", "state_backup_before_restore.json")

        # Security: Validate contents
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            for member in zip_ref.namelist():
                if not member.endswith(".json"):
                    raise Exception("Invalid file in ZIP. Only JSON files are allowed.")
            zip_ref.extractall(".")

        # Reload config/state into memory if needed
        load_config()
        load_state()
        ...
        await context.bot.edit_message_text(
            ...
            text="✅ Backup restored successfully!\nPlease restart the bot on Railway or your hosting service."
        )
