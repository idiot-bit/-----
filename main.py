...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # First save the document basic info
    document = update.message.document
    if not document:
        return

    file_id = document.file_id
    file_name = document.file_name or f"APK_{random.randint(1000,9999)}.apk"
    ...
    # --- Now Process According to Method ---
    if method == "method1":
...
async def method2_show_preview(user_id, context):
    ...
    if not session_files or not key:
        await context.bot.send_message(
            chat_id=user_id,
            text="⚠️ *No active APK session found!*",
            parse_mode="Markdown"
        )
        return

    preview_text = "🔖 *Captured APKs Preview:*\n\n"

    for idx, file_id in enumerate(session_files, start=1):
        try:
            file_name = session_filenames[idx-1] if idx-1 < len(session_filenames) else f"APK {idx}"
        except Exception:
            file_name = f"APK {idx}"

        if idx == len(session_files):
            # Last APK - Real saved caption
